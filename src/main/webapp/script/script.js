...
function edit_task(task_id){
    ...
    let save_tag = "<button id='save_" + task_id + "'>Safe</button>";
    $(identifier_edit).html(save_tag);
    let property_save_tag = "update_task(" + task_id + ")";
    $(identifier_edit).attr("onclick", property_save_tag);

    let td_description = $("#descciption_" + task_id);
    td_description.html("<input id='input_description_" + task_id + "' type = 'text' value='" + td_description.html() + "'>");
    console.log("cache check");
    let td_status = $("#status_" + task_id);
    let status_id = "#select-status_" + task_id;
    let status_current_value = td_status.html();
